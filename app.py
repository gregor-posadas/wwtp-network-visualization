...
def generate_targeted_network_diagram_streamlit(process_labels, dataframes, progress_bar, status_text, progress_increment, n_iterations=500, alpha=0.05):
    ...
    if st.button("Generate Targeted Network Diagram"):
        ...
        pearson_corr = bootstrap_correlations(combined_df, n_iterations=n_iterations, method='pearson', progress_bar=progress_bar, status_text=status_text, start_progress=0.0, end_progress=0.3 * progress_increment)
        spearman_corr = bootstrap_correlations(combined_df, n_iterations=n_iterations, method='spearman', progress_bar=progress_bar, status_text=status_text, start_progress=0.3 * progress_increment, end_progress=0.6 * progress_increment)
        kendall_corr = bootstrap_correlations(combined_df, n_iterations=n_iterations, method='kendall', progress_bar=progress_bar, status_text=status_text, start_progress=0.6 * progress_increment, end_progress=0.9 * progress_increment)
...
def main():
    ...
    # Define progress fractions
    heatmaps_start = 0.0
    heatmaps_end = 0.4
    network_diagrams_start = 0.4
    network_diagrams_end = 0.7
    bar_chart_start = 0.7
    bar_chart_end = 0.8
    line_graph_start = 0.8
    line_graph_end = 0.9
    targeted_network_start = 0.9
    targeted_network_end = 1.0
    ...
    if uploaded_files:
        ...
        heatmap_progress = 0.4  # 40%
        heatmap_step = heatmap_progress / num_heatmaps

        network_diagram_progress = 0.3  # 30%
        network_diagram_step = network_diagram_progress / 2  # Two diagrams

        bar_chart_progress = 0.1  # 10%
        line_graph_progress = 0.1  # 10%
        targeted_network_progress = 0.1  # 10%
        ...
        for i in range(len(dataframes) - 1):
            ...
            # Calculate start and end progress for this heatmap
            start_progress = heatmaps_start + i * heatmap_step
            end_progress = start_progress + heatmap_step

            # Update status
            status_text.text(f"Generating heatmap {i+1}/{num_heatmaps}...")

            # Generate heatmap with progress
        ...
        with col1:
            if st.button("Generate Globally Shared Network Diagram"):
                # Calculate start and end progress
                start_progress = network_diagrams_start
                end_progress = network_diagrams_start + network_diagram_step

                # Update status
                status_text.text("Generating Globally Shared Network Diagram...")
        ...
        with col2:
            if st.button("Generate Locally Shared Network Diagram"):
                # Calculate start and end progress
                start_progress = network_diagrams_start + network_diagram_step
                end_progress = network_diagrams_start + 2 * network_diagram_step

                # Update status
                status_text.text("Generating Locally Shared Network Diagram...")
        ...
        with col3:
            if st.button("Generate Bar Chart for Globally Shared Parameters"):
                # Calculate start and end progress
                start_progress = bar_chart_start
                end_progress = bar_chart_end

                # Update status
                status_text.text("Generating Bar Chart for Globally Shared Parameters...")

                plot_gspd_bar_chart(process_labels, globally_shared_parameters, correlation_matrices, progress_bar, status_text, end_progress - start_progress)

        with col4:
            if st.button("Generate Line Graph for Globally Shared Parameters"):
                # Calculate start and end progress
                start_progress = line_graph_start
                end_progress = line_graph_end

                # Update status
                status_text.text("Generating Line Graph for Globally Shared Parameters...")

                plot_gspd_line_graph(process_labels, globally_shared_parameters, correlation_matrices, progress_bar, status_text, end_progress - start_progress)
        ...
        generate_targeted_network_diagram_streamlit(process_labels, dataframes, progress_bar, status_text, targeted_network_progress)
